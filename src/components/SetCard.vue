<template>
  <div class="card" @click="$emit('card-clicked')">
    <svg id="pattern">
      ...
      >
        <circle cx="1" cy="1" r="2" fill="green" />
      </pattern>
      <pattern
        id="transformedPattern-blue"
        x="0"
        y="0"
        width="2"
        height="20"
        ...
        patternTransform="rotate(45)"
      >
        <circle cx="1" cy="1" r="2" fill="blue" />
      </pattern>
      <pattern
        id="transformedPattern-red"
        x="0"
        y="0"
        width="2"
        height="20"
...
export default {
  ...
  computed: {
    ...
    fill: function() {
      if (this.card[2] == "striped") {
        return `url(#transformedPattern-${this.card[1]})`;
      } else if (this.card[2] == "filled") {
        return this.card[1];
      } else {
        return "white";
      }
